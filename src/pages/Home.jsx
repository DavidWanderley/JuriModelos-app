...
const Home = () => {
  ...
  const modelosFiltrados = modelos.filter((m) => {
    ...
    const matchComplexidade =
      complexidadeAtiva === "Todas" || m.complexidade === complexidadeAtiva;
    ...
    <div className="ml-15 pt-10 p-10 bg-slate-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
          ...
          <h2 className="text-4xl font-black text-[#0e1e3f]">
            {modelos.length}
          </h2>
        </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center py-20">
          ...
          <p className="mt-4 text-slate-500 font-bold text-lg">
            Sincronizando com CW Advocacia...
          </p>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-20 border-2 border-dashed border-slate-200 text-center">
          <p className="text-xl font-medium text-slate-400">
            Nenhum documento encontrado nesta seção.
          </p>
        </div>
      )}
    </div>
