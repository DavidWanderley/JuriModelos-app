...
const Home = () => {
  ...
  return (
    <div className="ml-15 pt-10 p-10 bg-slate-50 min-h-screen">
      ...
      <div className="flex gap-4 mb-10 border-b border-slate-200 pb-6">
        {categorias.map((cat) => (
          <button
            key={cat}
            ...
            className={`px-6 py-2 rounded-lg text-base font-bold transition-all ${
              categoriaAtiva === cat 
                ? "bg-[#0e1e3f] text-white shadow-lg" 
      ...
      {loading ? (
        <div className="flex flex-col items-center py-20">
          ...
          <p className="mt-4 text-slate-500 font-bold text-lg">Sincronizando com CW Advocacia...</p>
        </div>
        ...
        <div className="bg-white rounded-[2rem] p-20 border-2 border-dashed border-slate-200 text-center">
          <p className="text-xl font-medium text-slate-400">Nenhum documento encontrado nesta seção.</p>
        </div>
      )}
    </div>
  );
